...
describe("Group Related Tests", function() {
	...
	before(function(done) { //create group to delete using user 1
		this.timeout(5000)
		chai.request(server)
			.post("/group/createGroup")
			.set("Authorization", "Bearer " + userToken1)
			...
			.end(function(err, res) {
				...
				res.body.should.be.a("object");
				res.body.should.have.property("groupInfo");
				groupIdToDelete = res.body.groupInfo.id;
				done();
			});
	});
    ...
    describe("PUT /group/editGroup", function() {
		...
		var groupEditDescriptionForm = {
			description: "New description"
		}

        it("Successful group edit (editGroup)", function(done) {
            chai.request(server)
		...
		it("Successful group edit (editGroupDescription)", function(done) {
			chai.request(server)
				.put("/group/editGroupDescription?groupId=" + groupId)
				.set("Authorization", "Bearer " + userToken1)
				.send(groupEditDescriptionForm)
				.end(function(err, res) {
					res.should.have.status(200);
					res.body.should.be.a("object");
					res.body.should.have.property("message");
					res.body.message.should.be.eql("Successful group description edit")
					done();
				});
		});
    ...
    describe("POST /group/joingroup and PUT /group/removeMember", function() {
        ...
        //Test POST /group/getAvailabilities
        describe("POST /group/getAvailabilities", function() {
            var getAvailabilitiesForm = {
                userThreshold: 1,
                timeThreshold: 0.5,
                day: "Mon",
            ...
            it("Successful getAvailabilities call", function(done) {
                chai.request(server)
                    .post("/group/getAvailabilities?groupId=" + groupId)
                    .set("Authorization", "Bearer " + userToken1)
                    .send(getAvailabilitiesForm)
                    .end(function(err, res) {
                        res.should.have.status(200);
                        res.body.should.be.a("object");
                        res.body.should.have.property("message");
                        res.body.message.should.be.eql("Successful availabilities calculation");
						res.body.should.have.property("freeTimes");
						res.body.freeTimes.should.have.length(7);
		...
		//Test POST /group/destroyGroup
		describe("DELETE /group/destroyGroup", function() {

			it("Successful group deletion", function(done) {
				chai.request(server)
					.delete("/group/destroyGroup?groupId=" + groupIdToDelete)
					.set("Authorization", "Bearer " + userToken1)
					.end(function(err, res) {
						res.should.have.status(200);
						res.body.should.be.a("object");
						res.body.should.have.property("message");
						res.body.message.should.be.eql("Successful group destroy");
						done();
					});
			});
		});
