...
let _callMethodAsync = (() => {
  var _ref = _asyncToGenerator(function* (url, method, requestBody, requestOptions, returnEntireResponse = false) {
    const clientId = yield (_Session || _load_Session()).clientIdAsync();
    const session = yield (_User || _load_User()).default.getSessionAsync();
...
let _downloadAsync = (() => {
  var _ref3 = _asyncToGenerator(function* (url, outputPath, progressFunction, retryFunction) {
    ...
    let { cancel, token } = (_axios || _load_axios()).default.CancelToken.source();
...
class ApiClient {
  ...
  static callMethodAsync(methodName, args, method, requestBody, requestOptions = {}, returnEntireResponse = false) {
  ...
  static sdkVersionsAsync() {
    return _asyncToGenerator(function* () {
      const { sdkVersions } = yield ApiClient.versionsAsync();
      return sdkVersions;
    })();
  }

  static turtleSdkVersionsAsync() {
    return _asyncToGenerator(function* () {
      const { turtleSdkVersions } = yield ApiClient.versionsAsync();
...
ApiClient._versionCache = new (_FsCache || _load_FsCache()).Cacher(() => ApiClient.callPathAsync('/--/api/v2/versions'), 'versions.json', 0, _path.default.join(__dirname, '../caches/versions.json'));
