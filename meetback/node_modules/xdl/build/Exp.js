...
let _downloadStarterAppAsync = (() => {
  var _ref2 = _asyncToGenerator(function* (templateId, progressFunction, retryFunction) {
    ...
    let templateApp = versions.templatesv2.find(function (template) {
    ...
    let starterAppPath = _path.default.join(_starterAppCacheDirectory(), `${templateId}-${templateApp.version}.tar.gz`);

    if (yield (_fsExtra || _load_fsExtra()).default.exists(starterAppPath)) {
    ...
    yield (_Api || _load_Api()).default.downloadAsync(templateApp.url, starterAppPath, {}, progressFunction, retryFunction);
    return starterAppPath;
  });
...
let downloadTemplateApp = exports.downloadTemplateApp = (() => {
  var _ref3 = _asyncToGenerator(function* (templateId, selectedDir, opts) {
    ...
    try {
      ...
      stats = (_fsExtra || _load_fsExtra()).default.statSync(root);
    } catch (e) {
      stats = null;
    }
    // This check is required because without it, the retry button would throw an error because the directory already exists,
    // even though it is empty.
    if (stats && !(stats.isDirectory() && (_fsExtra || _load_fsExtra()).default.readdirSync(root).length === 0)) {
      throw new (_XDLError || _load_XDLError()).default((_ErrorCode || _load_ErrorCode()).default.DIRECTORY_ALREADY_EXISTS, `The path "${root}" already exists.\nPlease choose a different parent directory or project name.`);
    }
...
let extractTemplateApp = exports.extractTemplateApp = (() => {
  var _ref4 = _asyncToGenerator(function* (starterAppPath, name, root) {
    ...
    // Update app.json
    let appJson = yield (_fsExtra || _load_fsExtra()).default.readFile(_path.default.join(root, 'app.json'), 'utf8');
    let customAppJson = appJson.replace(/\"My New Project\"/, `"${name}"`).replace(/\"my-new-project\"/, `"${name}"`);
    yield (_fsExtra || _load_fsExtra()).default.writeFile(_path.default.join(root, 'app.json'), customAppJson, 'utf8');
...
let getThirdPartyInfoAsync = exports.getThirdPartyInfoAsync = (() => {
  var _ref8 = _asyncToGenerator(function* (publicUrl) {
  ...
  return function getThirdPartyInfoAsync(_x14) {
...
let getPublishInfoAsync = exports.getPublishInfoAsync = (() => {
  var _ref9 = _asyncToGenerator(function* (root) {
    const user = yield (_User || _load_User()).default.ensureLoggedInAsync();
  ...
  return function getPublishInfoAsync(_x15) {
...
let sendAsync = exports.sendAsync = (() => {
  var _ref11 = _asyncToGenerator(function* (recipient, url_, allowUnauthed = true) {
    let result = yield (_Api || _load_Api()).default.callMethodAsync('send', [recipient, url_, allowUnauthed]);
    return result;
  });

  return function sendAsync(_x16, _x17) {
...
let getProjectRandomnessAsync = exports.getProjectRandomnessAsync = (() => {
  ...
  return function getProjectRandomnessAsync(_x18) {
...
let resetProjectRandomnessAsync = exports.resetProjectRandomnessAsync = (() => {
  ...
  return function resetProjectRandomnessAsync(_x19) {
