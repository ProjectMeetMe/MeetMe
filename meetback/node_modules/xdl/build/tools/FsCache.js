...
class Cacher {
  ...
  getAsync() {
    ...
    return _asyncToGenerator(function* () {
      yield mkdirpAsync(getCacheDir());

      let mtime;
      try {
        const stats = yield (_fsExtra || _load_fsExtra()).default.stat(_this.filename);
        mtime = stats.mtime;
      } catch (e) {
        if (_this.bootstrapFile) {
          try {
            const bootstrapContents = (yield (_fsExtra || _load_fsExtra()).default.readFile(_this.bootstrapFile)).toString();
            yield (_fsExtra || _load_fsExtra()).default.writeFile(_this.filename, bootstrapContents, 'utf8');
          } catch (e) {
            // intentional no-op
          }
        }
      ...
      if (new Date() - mtime > _this.ttlMilliseconds) {
        try {
          ...
          try {
            yield (_fsExtra || _load_fsExtra()).default.writeFile(_this.filename, JSON.stringify(fromCache), 'utf8');
          } catch (e) {
            _this.writeError = e;
      ...
      if (!fromCache) {
        try {
          fromCache = JSON.parse((yield (_fsExtra || _load_fsExtra()).default.readFile(_this.filename)));
        } catch (e) {
          _this.readError = e;
  ...
  clearAsync() {
    ...
    return _asyncToGenerator(function* () {
      try {
        yield (_fsExtra || _load_fsExtra()).default.unlink(_this2.filename);
      } catch (e) {
        _this2.writeError = e;
      }
    })();
